...
class CourseAdminController extends Controller
{
    ...
    //Generacion de constancias por el administrador
    public function getConstanciaUser($id,$id_user,Request $request)
    {
        // $id_user = Auth::user()->id;
        return $this->generarContancia($id,$id_user);
    }
    /*COnstancia para instructor*/
    public function getConstanciaInstructor($id,$id_user)
    {
        //$id_user = Auth::user()->id;
        return $this->generarContancia($id,$id_user);
    }
    ...
    public function index(Request $request)
    {
        ...
        $courses = Course::where('keywords', 'like', "%$search%")
            ...
            ->orderBy($request->input('order', 'name'), $request->input('orderType', 'desc'));

        $n = $courses->count();
        $courses = $courses->take($request->input('limit', 5))
            ->skip($request->input('page', 0) * $request->input('limit', 5))
            ->get();
    ...
    public function show($id, Request $request)
    {
        ...
        //return response()->json(['data' => $course, 'author' => $course);
        $id_user        =  decrypt($request->input('id'));
    ...
    public function update($id, Request $request)
    {
        ...
        $course->lng = $request->input('lng', $course->lng);
